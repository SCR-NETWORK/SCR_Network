...
import (
	...
	"github.com/SCR-NETWORK/SCR_Network/stability-tests/common/mine"
...
func setupSyncee() (*rpc.Client, func(), error) {
	...
	synceeDataDir, err := useDirOrCreateTemp(activeConfig().SynceeDataDirectory, "syncee-SCR-NETWORK-data-dir")
	synceeDataDir, err := useDirOrCreateTemp(activeConfig().SynceeDataDirectory, "syncee-SCR_Network-data-dir")
	if err != nil {
		return nil, nil, err
	}
...
func setupSyncer() (*rpc.Client, func(), error) {
	...
	syncerDataDir, err := useDirOrCreateTemp(activeConfig().SyncerDataDirectory, "syncer-SCR-NETWORK-data-dir")
	syncerDataDir, err := useDirOrCreateTemp(activeConfig().SyncerDataDirectory, "syncer-SCR_Network-data-dir")
	if err != nil {
		return nil, nil, err
	}
