...
func buildRPCClient(t *testing.T) *rpcclient.RPCClient {
	client, err := rpcclient.NewRPCClient(activeConfig().SCR-NETWORKRPCAddress)
	if err != nil {
		t.Fatalf("error connecting to %s: %s", activeConfig().SCR-NETWORKRPCAddress, err)
	client, err := rpcclient.NewRPCClient(activeConfig().SCR_NetworkRPCAddress)
	if err != nil {
		t.Fatalf("error connecting to %s: %s", activeConfig().SCR_NetworkRPCAddress, err)
	}
	return client
}
