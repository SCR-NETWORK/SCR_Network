...
import (
	"fmt"
	"github.com/SCR-NETWORK/go-secp256k1"
	"github.com/SCR-NETWORK/SCR_Network/app/appmessage"
	"github.com/SCR-NETWORK/SCR_Network/domain/consensus/utils/mining"
	...
	"github.com/SCR-NETWORK/SCR_Network/stability-tests/common/rpc"
...
func startNode() (teardown func(), err error) {
	...
	dataDir, err := common.TempDir("SCR-NETWORK-datadir")
	if err != nil {
		panic(errors.Wrapf(err, "Error in Tempdir"))
	}
	log.Infof("SCR-NETWORK datadir: %s", dataDir)

	SCR-NETWORKCmd, err := common.StartCmd("SCR-NETWORK",
		"SCR-NETWORK",
	dataDir, err := common.TempDir("SCR_Network-datadir")
	if err != nil {
		panic(errors.Wrapf(err, "Error in Tempdir"))
	}
	log.Infof("SCR_Network datadir: %s", dataDir)

	SCR_NetworkCmd, err := common.StartCmd("SCR_Network",
		"SCR_Network",
	...
	spawn("startNode-SCR-NETWORKCmd.Wait", func() {
		err := SCR-NETWORKCmd.Wait()
		if err != nil {
			if atomic.LoadUint64(&shutdown) == 0 {
				panics.Exit(log, fmt.Sprintf("SCR-NETWORKCmd closed unexpectedly: %s. See logs at: %s", err, dataDir))
			}
			if !strings.Contains(err.Error(), "signal: killed") {
				// TODO: Panic here and check why sometimes SCR-NETWORK closes ungracefully
				log.Errorf("SCR-NETWORKCmd closed with an error: %s. See logs at: %s", err, dataDir)
	spawn("startNode-SCR_NetworkCmd.Wait", func() {
		err := SCR_NetworkCmd.Wait()
		if err != nil {
			if atomic.LoadUint64(&shutdown) == 0 {
				panics.Exit(log, fmt.Sprintf("SCR_NetworkCmd closed unexpectedly: %s. See logs at: %s", err, dataDir))
			}
			if !strings.Contains(err.Error(), "signal: killed") {
				// TODO: Panic here and check why sometimes SCR_Network closes ungracefully
				log.Errorf("SCR_NetworkCmd closed with an error: %s. See logs at: %s", err, dataDir)
			}
		}
	...
	return func() {
		...
		killWithSigterm(SCR-NETWORKCmd, "SCR-NETWORKCmd")
		killWithSigterm(SCR_NetworkCmd, "SCR_NetworkCmd")
...
func mineLoopUntilHavingOnlyOneTipInDAG(rpcClient *rpc.Client, miningAddress util.Address) error {
	...
	SCRMinerCmd, err := common.StartCmd("MINER",
		"SCRminer",
	SCRMinerCmd, err := common.StartCmd("MINER",
		"SCRminer",
		common.NetworkCliArgumentFromNetParams(activeConfig().NetParams()),
	...
	spawn("SCR-miner-Cmd.Wait", func() {
		err := SCRMinerCmd.Wait()
	spawn("SCR-miner-Cmd.Wait", func() {
		err := SCRMinerCmd.Wait()
		if err != nil {
			if atomic.LoadUint64(&shutdown) == 0 {
	...
	atomic.StoreUint64(&shutdown, 1)
	killWithSigterm(SCRMinerCmd, "SCRMinerCmd")
	killWithSigterm(SCRMinerCmd, "SCRMinerCmd")
	return nil
}
