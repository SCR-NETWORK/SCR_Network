...
func startNodes() (teardown func(), err error) {
	...
	syncerDataDir, err := common.TempDir("SCR-NETWORK-datadir-syncer")
	syncerDataDir, err := common.TempDir("SCR_Network-datadir-syncer")
	if err != nil {
		panic(errors.Wrapf(err, "error in Tempdir"))
	}
	log.Infof("SYNCER datadir: %s", syncerDataDir)

	syncedDataDir, err := common.TempDir("SCR-NETWORK-datadir-synced")
	syncedDataDir, err := common.TempDir("SCR_Network-datadir-synced")
	if err != nil {
		panic(errors.Wrapf(err, "error in Tempdir"))
	}
	log.Infof("SYNCED datadir: %s", syncedDataDir)

	syncerCmd, err := common.StartCmd("SCR-NETWORK-SYNCER",
		"SCR-NETWORK",
	syncerCmd, err := common.StartCmd("SCR_Network-SYNCER",
		"SCR_Network",
	...
	syncedCmd, err := common.StartCmd("SCR-NETWORK-SYNCED",
		"SCR-NETWORK",
	syncedCmd, err := common.StartCmd("SCR_Network-SYNCED",
		"SCR_Network",
		common.NetworkCliArgumentFromNetParams(activeConfig().NetParams()),
