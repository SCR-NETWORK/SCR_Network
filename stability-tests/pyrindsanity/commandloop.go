...
func commandLoop(argsChan <-chan []string) ([]commandFailure, error) {
	...
	dataDirectoryPath, err := common.TempDir("SCR-NETWORKsanity-SCR-NETWORK-datadir")
	dataDirectoryPath, err := common.TempDir("SCR_Networksanity-SCR_Network-datadir")
	if err != nil {
	...
	for args := range argsChan {
		...
		cmd := exec.Command("SCR-NETWORK", args...)
		cmd.Stdout = common.NewLogWriter(log, logger.LevelTrace, "SCR-NETWORK-STDOUT")
		cmd.Stderr = common.NewLogWriter(log, logger.LevelWarn, "SCR-NETWORK-STDERR")
		cmd := exec.Command("SCR_Network", args...)
		cmd.Stdout = common.NewLogWriter(log, logger.LevelTrace, "SCR_Network-STDOUT")
		cmd.Stderr = common.NewLogWriter(log, logger.LevelWarn, "SCR_Network-STDERR")
