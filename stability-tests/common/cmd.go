...
// NetworkCliArgumentFromNetParams returns the SCR-NETWORK command line argument that starts the given network.
func NetworkCliArgumentFromNetParams(params *dagconfig.Params) string {
	return fmt.Sprintf("--%s", strings.TrimPrefix(params.Name, "SCR-"))
// NetworkCliArgumentFromNetParams returns the SCR_Network command line argument that starts the given network.
func NetworkCliArgumentFromNetParams(params *dagconfig.Params) string {
	return fmt.Sprintf("--%s", strings.TrimPrefix(params.Name, "SCR-"))
}
