...
func TestUTXOIndex(t *testing.T) {
	...
	err := SCR-NETWORK.rpcClient.RegisterForUTXOsChangedNotifications([]string{miningAddress1}, func(
	err := SCR_Network.rpcClient.RegisterForUTXOsChangedNotifications([]string{miningAddress1}, func(
	...
	getCoinSupplyResponse, err := SCR-NETWORK.rpcClient.GetCoinSupply()
		mineNextBlock(t, SCR_Network)
	}

	//check if rewards corrosponds to circulating supply.
	getCoinSupplyResponse, err := SCR_Network.rpcClient.GetCoinSupply()
	if err != nil {
		t.Fatalf("Error Retriving Coin supply: %s", err)
	}

	rewardsMinedLeor := uint64(blockAmountToMine * constants.LeorPerSCR * 17)
	getBlockCountResponse, err := SCR-NETWORK.rpcClient.GetBlockCount()
	getBlockCountResponse, err := SCR_Network.rpcClient.GetBlockCount()
	if err != nil {
		t.Fatalf("Error Retriving BlockCount: %s", err)
	}
	rewardsMinedViaBlockCountLeor := uint64(
		(getBlockCountResponse.BlockCount - 2) * constants.LeorPerSCR * 17, // -2 because of genesis and virtual.
	)
	...
	for i := 0; i < transactionAmountToSpend; i++ {
		...
		_, err = SCR-NETWORK.rpcClient.SubmitTransaction(rpcTransaction, false)
		_, err = SCR_Network.rpcClient.SubmitTransaction(rpcTransaction, false)
		if err != nil {
	...
	utxosByAddressesResponse, err := SCR-NETWORK.rpcClient.GetUTXOsByAddresses([]string{miningAddress1})
	utxosByAddressesResponse, err := SCR_Network.rpcClient.GetUTXOsByAddresses([]string{miningAddress1})
	if err != nil {
...
func buildTransactionForUTXOIndexTest(t *testing.T, entry *appmessage.UTXOsByAddressesEntry) *appmessage.RPCTransaction {
	...
	payeeAddress, err := util.DecodeAddress(miningAddress1, util.Bech32PrefixSCRSim)
	if err != nil {
