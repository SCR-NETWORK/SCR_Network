...
func TestVirtualSelectedParentChain(t *testing.T) {
	// Setup a couple of SCR-NETWORK instances
	SCR-NETWORK1, SCR-NETWORK2, _, teardown := standardSetup(t)
	// Setup a couple of SCR_Network instances
	SCR_Network1, SCR_Network2, _, teardown := standardSetup(t)
	defer teardown()
	...
	err := SCR-NETWORK1.rpcClient.RegisterForVirtualSelectedParentChainChangedNotifications(true,
	err := SCR_Network1.rpcClient.RegisterForVirtualSelectedParentChainChangedNotifications(true,
	...
	// In SCR-NETWORK1, mine a chain over the genesis and make sure
	// each chain changed notifications contains only one entry
	// in `added` and nothing in `removed`
	chain1TipHash := consensushashing.BlockHash(SCR-NETWORK1.config.NetParams().GenesisBlock)
	chain1TipHashString := chain1TipHash.String()
	const blockAmountToMine = 10
	for i := 0; i < blockAmountToMine; i++ {
		minedBlock := mineNextBlock(t, SCR-NETWORK1)
	// In SCR_Network1, mine a chain over the genesis and make sure
	// each chain changed notifications contains only one entry
	// in `added` and nothing in `removed`
	chain1TipHash := consensushashing.BlockHash(SCR_Network1.config.NetParams().GenesisBlock)
	chain1TipHashString := chain1TipHash.String()
	const blockAmountToMine = 10
	for i := 0; i < blockAmountToMine; i++ {
		minedBlock := mineNextBlock(t, SCR_Network1)
	...
	// In SCR-NETWORK2, mine a different chain of `blockAmountToMine` + 1
	// blocks over the genesis
	var chain2Tip *externalapi.DomainBlock
	for i := 0; i < blockAmountToMine+1; i++ {
		chain2Tip = mineNextBlock(t, SCR-NETWORK2)
	}

	// Connect the two SCR-NETWORKs. This should trigger sync
	// between the two nodes
	connect(t, SCR-NETWORK1, SCR-NETWORK2)
	// In SCR_Network2, mine a different chain of `blockAmountToMine` + 1
	// blocks over the genesis
	var chain2Tip *externalapi.DomainBlock
	for i := 0; i < blockAmountToMine+1; i++ {
		chain2Tip = mineNextBlock(t, SCR_Network2)
	}

	// Connect the two SCR_Networks. This should trigger sync
	// between the two nodes
	connect(t, SCR_Network1, SCR_Network2)
	...
	// the first chain
	virtualSelectedParentChainFromChain1Tip, err := SCR-NETWORK1.rpcClient.GetVirtualSelectedParentChainFromBlock(
	virtualSelectedParentChainFromChain1Tip, err := SCR_Network1.rpcClient.GetVirtualSelectedParentChainFromBlock(
