...
// long-running SCR-NETWORKMain (which is the real meat of SCR-NETWORK), handles service
// long-running SCR_NetworkMain (which is the real meat of SCR_Network), handles service
...
func (s *Service) Execute(args []string, r <-chan svc.ChangeRequest, changes chan<- svc.Status) (bool, uint32) {
	...
	// Start SCR-NETWORKMain in a separate goroutine so the service can start
	// quickly. Shutdown (along with a potential error) is reported via
	// doneChan. startedChan is notified once SCR-NETWORK is started so this can
	// be properly logged
	doneChan := make(chan error)
	startedChan := make(chan struct{})
	spawn("SCR-NETWORKMain-windows", func() {
	// Start SCR_NetworkMain in a separate goroutine so the service can start
	// quickly. Shutdown (along with a potential error) is reported via
	// doneChan. startedChan is notified once SCR_Network is started so this can
	// be properly logged
	doneChan := make(chan error)
	startedChan := make(chan struct{})
	spawn("SCR_NetworkMain-windows", func() {
...
// logServiceStart logs information about SCR-NETWORK when the main server has
// logServiceStart logs information about SCR_Network when the main server has
