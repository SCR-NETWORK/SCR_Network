...
Clients are expected to build RequestMessages and wrap them in SCRpadMessage. (see messages.proto)

Having received a RequestMessage, (wrapped in a SCRpadMessage) the RPC server will respond with a
ResponseMessage (likewise wrapped in a SCRpadMessage) respective to the original RequestMessage.
...
GetCurrentNetworkRequestMessage requests the network SCR-NETWORK is currently running against.
GetCurrentNetworkRequestMessage requests the network SCR_Network is currently running against.
...
| Field | Type | Label | Description |
| ----- | ---- | ----- | ----------- |
| payAddress | [string](#string) |  | Which SCR address should the coinbase block reward transaction pay into |
| payAddress | [string](#string) |  | Which SCR address should the coinbase block reward transaction pay into |
...
| ----- | ---- | ----- | ----------- |
| block | [RpcBlock](#protowire.RpcBlock) |  |  |
| isSynced | [bool](#bool) |  | Whether SCR-NETWORK thinks that it&#39;s synced. Callers are discouraged (but not forbidden) from solving blocks when SCR-NETWORK is not synced. That is because when SCR-NETWORK isn&#39;t in sync with the rest of the network there&#39;s a high chance the block will never be accepted, thus the solving effort would have been wasted. |
| isSynced | [bool](#bool) |  | Whether SCR_Network thinks that it&#39;s synced. Callers are discouraged (but not forbidden) from solving blocks when SCR_Network is not synced. That is because when SCR_Network isn&#39;t in sync with the rest of the network there&#39;s a high chance the block will never be accepted, thus the solving effort would have been wasted. |
| error | [RPCError](#protowire.RPCError) |  |  |
...
GetPeerAddressesRequestMessage requests the list of known SCR-NETWORK addresses in the
GetPeerAddressesRequestMessage requests the list of known SCR_Network addresses in the
...
currently connected to this SCR-NETWORK.
currently connected to this SCR_Network.
...
| isOutbound | [bool](#bool) |  | Whether this SCR-NETWORK initiated the connection |
| timeOffset | [int64](#int64) |  |  |
| userAgent | [string](#string) |  |  |
| advertisedProtocolVersion | [uint32](#uint32) |  | The protocol version that this peer claims to support |
| timeConnected | [int64](#int64) |  | The timestamp of when this peer connected to this SCR-NETWORK |
| isOutbound | [bool](#bool) |  | Whether this SCR_Network initiated the connection |
| timeOffset | [int64](#int64) |  |  |
| userAgent | [string](#string) |  |  |
| advertisedProtocolVersion | [uint32](#uint32) |  | The protocol version that this peer claims to support |
| timeConnected | [int64](#int64) |  | The timestamp of when this peer connected to this SCR_Network |
...
AddPeerRequestMessage adds a peer to SCR-NETWORK&#39;s outgoing connection list.
This will, in most cases, result in SCR-NETWORK connecting to said peer.
AddPeerRequestMessage adds a peer to SCR_Network&#39;s outgoing connection list.
This will, in most cases, result in SCR_Network connecting to said peer.
...
parent chain from some startHash to this SCR-NETWORK&#39;s current virtual
parent chain from some startHash to this SCR_Network&#39;s current virtual
...
SCR-NETWORK&#39;s current virtual.
SCR_Network&#39;s current virtual.
...
GetBlockCountRequestMessage requests the current number of blocks in this SCR-NETWORK.
GetBlockCountRequestMessage requests the current number of blocks in this SCR_Network.
...
of this SCR-NETWORK&#39;s DAG.
of this SCR_Network&#39;s DAG.
...
ShutDownRequestMessage shuts down this SCR-NETWORK.
ShutDownRequestMessage shuts down this SCR_Network.
...
This call is only available when this SCR-NETWORK was started with `--utxoindex`
This call is only available when this SCR_Network was started with `--utxoindex`
...
GetUtxosByAddressesRequestMessage requests all current UTXOs for the given SCR-NETWORK addresses

This call is only available when this SCR-NETWORK was started with `--utxoindex`
GetUtxosByAddressesRequestMessage requests all current UTXOs for the given SCR_Network addresses
