...
import (
	...
	"github.com/SCR-NETWORK/SCR_Network/util/network"
...
const (
	defaultConfigFilename      = "SCR-NETWORK.conf"
	defaultLogLevel            = "info"
	defaultLogDirname          = "logs"
	defaultLogFilename         = "SCR-NETWORK.log"
	defaultErrLogFilename      = "SCR-NETWORK_err.log"
	defaultConfigFilename      = "SCR_Network.conf"
	defaultLogLevel            = "info"
	defaultLogDirname          = "logs"
	defaultLogFilename         = "SCR_Network.log"
	defaultErrLogFilename      = "SCR_Network_err.log"
	...
	sampleConfigFilename    = "sample-SCR-NETWORK.conf"
	sampleConfigFilename    = "sample-SCR_Network.conf"
	defaultMaxUTXOCacheSize = 5_000_000_000
	defaultProtocolVersion  = 5
)

var (
	// DefaultAppDir is the default home directory for SCR-NETWORK.
	DefaultAppDir = util.AppDir("SCR-NETWORK", false)
	// DefaultAppDir is the default home directory for SCR_Network.
	DefaultAppDir = util.AppDir("SCR_Network", false)
...
//go:embed sample-SCR-NETWORK.conf
//go:embed sample-SCR_Network.conf
...
// Flags defines the configuration options for SCR-NETWORK.
// Flags defines the configuration options for SCR_Network.
//
...
type Flags struct {
	...
	Listeners                       []string      `long:"listen" description:"Add an interface/port to listen for connections (default all interfaces port: 13111, testnet: 16211)"`
	...
	RPCListeners                    []string      `long:"rpclisten" description:"Add an interface/port to listen for RPC connections (default port: 13110, testnet: 16210)"`
	...
	MinRelayTxFee                   float64       `long:"minrelaytxfee" description:"The minimum transaction fee in SCR/kB to be considered a non-zero fee."`
	MinRelayTxFee                   float64       `long:"minrelaytxfee" description:"The minimum transaction fee in SCR/kB to be considered a non-zero fee."`
...
// Config defines the configuration options for SCR-NETWORK.
// Config defines the configuration options for SCR_Network.
//
// See loadConfig for details on the configuration load process.
type Config struct {
	*Flags
...
// DefaultConfig returns the default SCR-NETWORK configuration
// DefaultConfig returns the default SCR_Network configuration
...
//
// The above results in SCR-NETWORK functioning properly without any config settings
// The above results in SCR_Network functioning properly without any config settings
...
// createDefaultConfig copies the file sample-SCR-NETWORK.conf to the given destination path,
// createDefaultConfig copies the file sample-SCR_Network.conf to the given destination path,
