...
func TestCreateDefaultConfigFile(t *testing.T) {
	...
	sampleConfigFile := filepath.Join(filepath.Dir(path), "sample-SCR-NETWORK.conf")

	// Setup a temporary directory
	tmpDir, err := ioutil.TempDir("", "SCR-NETWORK")
	sampleConfigFile := filepath.Join(filepath.Dir(path), "sample-SCR_Network.conf")

	// Setup a temporary directory
	tmpDir, err := ioutil.TempDir("", "SCR_Network")
	if err != nil {
		t.Fatalf("Failed creating a temporary directory: %v", err)
	}
	testpath := filepath.Join(tmpDir, "test.conf")

	// copy config file to location of SCR-NETWORK binary
	// copy config file to location of SCR_Network binary
	...
	tmpConfigFile := filepath.Join(appPath, "sample-SCR-NETWORK.conf")
	tmpConfigFile := filepath.Join(appPath, "sample-SCR_Network.conf")
	err = ioutil.WriteFile(tmpConfigFile, data, 0644)
	if err != nil {
		t.Fatalf("Failed coSCRng sample config file: %v", err)
	}
