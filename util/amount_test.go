...
func TestAmountCreation(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			amount:   100,
			valid:    true,
			expected: 100 * constants.LeorPerSCR,
		},
		{
			name:     "fraction",
			amount:   0.01234567,
		...
		},
		{
			...
			valid:    true,
			expected: 55 * constants.LeorPerSCR,
		},
		{
			name:     "rounding down",
...
func TestAmountUnitConversions(t *testing.T) {
	...
	for _, test := range tests {
		...
		// Verify that Amount.ToSCR works as advertised.
		f1 := test.amount.ToUnit(AmountSCR)
		f2 := test.amount.ToSCR()
		if f1 != f2 {
			t.Errorf("%v: ToSCR does not match ToUnit(AmountSCR): %v != %v", test.name, f1, f2)
		}

		// Verify that Amount.String works as advertised.
		s1 := test.amount.Format(AmountSCR)
		s2 := test.amount.String()
		if s1 != s2 {
			t.Errorf("%v: String does not match Format(AmountSCR): %v != %v", test.name, s1, s2)
		// Verify that Amount.ToSCR works as advertised.
		f1 := test.amount.ToUnit(AmountSCR)
		f2 := test.amount.ToSCR()
		if f1 != f2 {
...
func TestAmountMulF64(t *testing.T) {
	tests := []struct {
		...
		{
			name: "Multiply 0.1 SCR by 2",
			amt:  100e5, // 0.1 SCR
			mul:  2,
			res:  200e5, // 0.2 SCR
		},
		{
			name: "Multiply 0.2 SCR by 0.02",
			amt:  200e5, // 0.2 SCR
			mul:  1.02,
			res:  204e5, // 0.204 SCR
			name: "Multiply 0.1 SCR by 2",
