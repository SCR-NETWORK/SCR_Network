...
import (
	...
	"github.com/SCR-NETWORK/SCR_Network/infrastructure/network/netadapter/server/grpcserver/protowire"

	"github.com/pkg/errors"
	"google.golang.org/protobuf/encoding/protojson"

	"github.com/SCR-NETWORK/SCR_Network/infrastructure/network/rpcclient/grpcclient"
)
...
func main() {
	...
	if !cfg.AllowConnectionToDifferentVersions {
		SCR-NETWORKMessage, err := client.Post(&protowire.PyipadMessage{Payload: &protowire.PyipadMessage_GetInfoRequest{GetInfoRequest: &protowire.GetInfoRequestMessage{}}})
		if err != nil {
			printErrorAndExit(fmt.Sprintf("Cannot post GetInfo message: %s", err))
		}

		localVersion := version.Version()
		remoteVersion := SCR-NETWORKMessage.GetGetInfoResponse().ServerVersion
...
func prettifyResponse(response string) string {
	SCR-NETWORKMessage := &protowire.PyipadMessage{}
	err := protojson.Unmarshal([]byte(response), SCR-NETWORKMessage)
	if err != nil {
	...
	return marshalOptions.Format(SCR-NETWORKMessage)
}
