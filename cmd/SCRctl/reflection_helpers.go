...
// generateSCR-NETWORKMessage generates a wrapped SCR-NETWORKMessage with the given `commandValue`
func generateSCR-NETWORKMessage(commandValue reflect.Value, commandDesc *commandDescription) (*protowire.PyipadMessage, error) {
	commandWrapper := reflect.New(commandDesc.typeof)
	unwrapCommandValue(commandWrapper).Set(commandValue)

	SCR-NETWORKMessage := reflect.New(reflect.TypeOf(protowire.PyipadMessage{}))
	SCR-NETWORKMessage.Elem().FieldByName("Payload").Set(commandWrapper)
	return SCR-NETWORKMessage.Interface().(*protowire.PyipadMessage), nil
}
