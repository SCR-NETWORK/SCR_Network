...
func (csm *consensusStateManager) isViolatingFinality(stagingArea *model.StagingArea, blockHash *externalapi.DomainHash,
	...
	if !isInSelectedParentChainOfFinalityPoint {
		...
		log.Debugf("Block %s violates finality, but SCR-NETWORK is currently doing IBD, so this is normal", blockHash)
		log.Debugf("Block %s violates finality, but SCR_Network is currently doing IBD, so this is normal", blockHash)
